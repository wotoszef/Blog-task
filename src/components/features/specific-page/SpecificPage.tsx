...
const SpecificPage = () => {
  ...
  return (
    <Box>
      <div style={{display: 'flex',flexDirection: 'column', alignItems: 'flex-start', margin: ' 0 2rem'}}>
        <h1 style={{ textAlign: "center", fontSize: '2rem', textTransform: 'capitalize' }}>{currentPost?.title}</h1>
        <p style={{ textAlign: "center" }}>{currentPost?.body}</p>
      </div>
      <Box style={{display: 'flex',flexDirection: 'column', alignItems: 'flex-start', margin: ' 0 2rem'}}>
        <h3 style={{ textAlign: "center", fontSize: '1.5rem' }}>Comments</h3>
        {commentsDetails.map((commentDetail: CommentDetailsProps, index: number) => {
